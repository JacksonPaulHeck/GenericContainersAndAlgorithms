...
template <typename T> void algo_swap(T &a, T &b) {
  T tmp = a;
  a = b;
  b = tmp;
}
...
template <typename RandomAccessIter>
RandomAccessIter algo_min_element(RandomAccessIter first,
...
struct Iter_less_val {
  ...
  Iter_less_val iter_comp_val(Iter_less_iter) { return Iter_less_val(); }
};

// Heap Sort Normal

template <typename RandomAccessIter, typename T, typename Compare>
void push_heap(RandomAccessIter first, long int hole, long int top, T val,
               Compare &comp) {
...
void pop_heap(RandomAccessIter first, RandomAccessIter last,
              RandomAccessIter result, Compare &comp) {
  auto val = *result;
  *result = *first;
...
template <typename RandomAccessIter, typename T, typename Compare>
void adjust_heap(RandomAccessIter first, long int hole, long int len, T val,
                 Compare comp) {
...
void pop_heap(RandomAccessIter first, RandomAccessIter last,
              RandomAccessIter result, Compare1 &comp, Compare2 cmp) {
  auto val = *result;
  *result = *first;
...
template <typename RandomAccessIter, typename Compare1, typename Compare2>
void pop_heap(RandomAccessIter first, RandomAccessIter last, Compare1 comp,
              Compare2 cmp) {
  if (last - first > 1) {
    --last;
...
template <typename RandomAccessIter, typename T, typename Compare1,
          typename Compare2>
void adjust_heap(RandomAccessIter first, long int hole, long int len, T val,
                 Compare1 comp, Compare2 cmp) {
  const long int top = hole;
  long int second = hole;
  while (second < (len - 1) / 2) {
    second = 2 * (second + 1);
...
template <typename RandomAccessIter, typename Compare1, typename Compare2>
void make_heap(RandomAccessIter first, RandomAccessIter last, Compare1 comp,
               Compare2 cmp) {
  if (last - first < 2) {
    return;
  }
...
template <typename RandomAccessIter, typename Compare1, typename Compare2>
void algo_heap_sort(RandomAccessIter begin, RandomAccessIter end, Compare1 comp,
                    Compare2 cmp) {
