...
struct outOfRangeVector : std::exception {
  const char *what() const noexcept override {
    return "Index is out of range for AlgoVector!";
  }
};
...
template <typename T> class AlgoVector {
  ...
  struct RandomAccessIterator {
    ...
    RandomAccessIterator operator+(const difference_type &movement) {
    ...
    difference_type operator-(const RandomAccessIterator &rawIterator) {
    ...
    //   friend bool operator< (RandomAccessIterator& a, RandomAccessIterator&
    //   b) { return a < b; }; friend bool operator> (RandomAccessIterator& a,
    //   RandomAccessIterator& b) { return a > b; };
    ...
    operator=(const RandomAccessIterator &rawIterator) = default;
    RandomAccessIterator &operator=(RandomAccessIterator *ptr) {
      m_ptr = ptr;
      return (*this);
    }
...
AlgoVector<T> &AlgoVector<T>::operator=(
    const AlgoVector<T>
        &algoVector) { // Assignment operator from another AlgoVector
  if (this != &algoVector) {
    delete[] data;
...
template <typename T>
T &AlgoVector<T>::at(int index) const { // subscript operator
  if (!(index < 0 || index >= capacity)) {
    return data[index];
  } else {
    throw outOfRangeVector();
...
template <typename T>
int AlgoVector<T>::size() { // check the size of the vector
  return capacity;
}
...
template <typename T> void AlgoVector<T>::print() {
  for (int i = 0; i < capacity - 1; i++) {
    std::cout << data[i] << " ";
  }
...
template <typename T> void AlgoVector<T>::print(std::ofstream &file) {
  for (int i = 0; i < capacity - 1; i++) {
    file << data[i] << " ";
  }
